...
static uint64_t riscv_load_fdt_smol(hwaddr dram_base, uint64_t mem_size, void *fdt)
{
    uint64_t fdt_addr;
    ...
    fdt_addr = QEMU_ALIGN_DOWN(dram_end - fdtsize, 4 * KiB);

    warn_report("fdt addr: 0x%08lx size: 0x%08x\n", fdt_addr, fdtsize);
...
static hwaddr riscv_load_initrd_smol(const char *filename, uint64_t mem_size,
...
static void virt_machine_done(Notifier *notifier, void *data)
{
    ...
    if (machine->kernel_filename) {
        ...
        if (machine->initrd_filename) {
            hwaddr start;
            hwaddr end = riscv_load_initrd_smol(machine->initrd_filename,
    ...
    fdt_load_addr = riscv_load_fdt_smol(memmap[VIRT_DRAM].base,
...
static const TypeInfo virt_machine_typeinfo = {
    .name       = MACHINE_TYPE_NAME("virt_smol"),
    .parent     = TYPE_MACHINE,
